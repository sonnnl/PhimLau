...
import MovieCard from "../components/MovieCard"; // Đảm bảo đường dẫn đúng
import {
  Container,
  Heading,
  SimpleGrid,
...
const SingleMoviesPage = () => {
  ...
  const fetchMovies = useCallback(async (currentPage) => {
    ...
    try {
      ...
      console.error("Error in fetchMovies (SingleMoviesPage):", err);
      setError(err.message || "Không thể tải danh sách phim lẻ.");
      setHasMore(false);
    } finally {
      if (currentPage === 1) {
    ...
    <Container maxW="container.xl" py={8}>
      ...
      >
        ...
        <BreadcrumbItem isCurrentPage>
          <BreadcrumbLink href="#">Phim Lẻ</BreadcrumbLink>
        </BreadcrumbItem>
      </Breadcrumb>
      ...
      {initialLoading ? (
        ...
        <Center minH="200px">
          <Text fontSize="xl" color="text.secondary">
            Không có phim lẻ nào để hiển thị.
          </Text>
        </Center>
      ) : (
        <InfiniteScroll
